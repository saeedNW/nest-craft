...
export async function errorHandler(error, parentDirectory, projectDirectory) {
  ...
  if (projectDirectory) {
    try {
      ...
      console.error(`Failed to remove project directory: ${projectDirectory.path}`, cleanupError);
    }
  }
  ...
  if (parentDirectory?.status && parentDirectory?.path) {
    try {
      ...
      // Log an error if directory removal fails.
      console.error(`Failed to remove parent directory: ${parentDirectory.path}`, cleanupError);
    }
  }
