 ...
 * @returns {Promise<boolean>}
 */
export async function manageAppModule(targetDirectory, loggerStatus) {
...
${loggerStatus ? 'import { LoggerModule } from "./modules/logger";' : ''}

@Module({
  imports: [
		${loggerStatus ? '// Logger module must be imported early to be available globally' : ''}
		${loggerStatus ? 'LoggerModule,' : ''}
    // Load ENVs
