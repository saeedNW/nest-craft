...
import { mainProjectPath } from "../functions/main-project-path.function.js";
...
export async function dockerComposeGenerator(projectPath, dockerComposeConfig) {
	...
	const dockerComposeContent = await generateDockerComposeContent(
		dockerComposeConfig,
		basePath
	);
...
async function createTempDockerComposeFile() {
	const tempDirectory = await fs.mkdtemp(
		path.join(os.tmpdir(), "docker-compose-")
	);
...
async function handleAdditionalFiles(projectPath, basePath, services) {
	...
	if (services.includes(ServicesName.node)) {
		copyTasks.push(
			copyFile(
				path.join(basePath, "config/Dockerfile"),
	...
	if (services.includes(ServicesName.nginx)) {
		copyTasks.push(
			copyFile(
				path.join(basePath, "config/nginx.conf"),
